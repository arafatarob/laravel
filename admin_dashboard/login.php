...
if (!empty($_POST)) {
    ...
    $sql = "SELECT id, role, name FROM users WHERE email='$email' AND password='$password' LIMIT 1";
    $result = $conn->query($sql);

    if ($result && $row = $result->fetch_assoc()) {
        $_SESSION['user_id'] = $row['id'];
        $_SESSION['user_email'] = $email;
        $_SESSION['user_role'] = strtolower($row['role'] ?? 'user');
...
<body>
        ...
        <div class="form-container">
            ...
            <form id="adminForm" method="post">
                ...
                <div class="form-footer">
                    Don't have an account? <a href="signup.php<?php echo $next ? '?next=' . urlencode($next) : ''; ?>">Register Now</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        // Form validation
        const validateForm = () => {
            const isValid = emailInput.value && passwordInput.value;
