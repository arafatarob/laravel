...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name || !$email || !$password) {
        ...
        if ($check && $check->num_rows > 0) {
            ...
            $insert = $conn->query("INSERT INTO users (name, email, password, role, created_at) VALUES ('$name', '$email', '$password', '$role', '$created_at')");
            if ($insert) {
                if ($next) {
...
<body>
    ...
    <script>
        ...
        // Form validation
        const validateForm = () => {
            const isValid = nameInput.value && emailInput.value && passwordInput.value;
